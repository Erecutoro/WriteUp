...
We can see that there is a check() function.
...
We can see in the check function a string comparison with element from the flag (actf{~~~}).

So if we reverse the swap functionality and put the unordered flag as the password we achive the following result.
